...
export const HDPATH = [44, 118, 0, 0, 0];
export const DIVISOR = 1000000000000000000;
...
export const getAccountInfo = async function (addr) {
    const url = `${LCD}/bank/accounts/${addr}`;
    ...
    return axios.get(url).then(
        r => {
            try {
                if (typeof r.data.value !== 'undefined') {
                    ...
                    if (r.data.value.coins !== null) {
                        txContext.balanceIris = r.data.value.coins[0].amount / DIVISOR;
                    }
                }
            } catch (e) {
