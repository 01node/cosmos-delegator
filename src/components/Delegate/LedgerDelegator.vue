<template>
  <div v-if="validator" class="col-12 col-md-8">
    <div class="card">
      ...
      <div class="card-body" v-if="address.bech32_address=== undefined">
      ...
      <div class="card-body" v-if="address.bech32_address!== undefined">
        ...
        <div v-if="confirmed === false">
          ...
          <div
            ...
            v-else
          >You cannot delegate because you have 0 IRIS tokens on the address.</div>
...
import { ERROR_CODE, HDPATH, getAccountInfo } from "@/utils/common";
...
export default {
  ...
  methods: {
    ...
    async getAccountInfo() {
      if (this.ledger.connected === true) {
        try {
          this.accountInfo = await getAccountInfo(this.address.bech32_address);
        } catch (error) {
          this.errorMessage =
    ...
    async signTransaction() {
      try {
        ...
        const response = await this.ledger.sign(HDPATH, signMessage);
        this.log("Response received!");
        this.log("Full response:");
        this.log(response);

        if (response.return_code === 36864) {
          const pubKeyBuffer = Buffer.from(this.address.compressed_pk, "hex");
    ...
    async generateTx() {
      const defaultTx = {
        fee: {
          amount: [{ denom: DENOM, amount: String(DEFAULT_FEE * DIVISOR) }],
          gas: String(50000),
          /*1 500 000 000 000
          6 000 000 000 000 */
        },
      ...
      txMessage["msg"] = [
        {
          ...
          value: {
            delegator_addr: this.address.bech32_address,
            validator_addr: this.operator_address,
            delegation: {
              denom: DENOM,
              amount: String(
                (this.delegateValue * 1000).toString() + "000000000000000"
              ),
            },
          },
        },
